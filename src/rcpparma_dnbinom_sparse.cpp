...
// [[Rcpp::export]]
Rcpp::NumericVector
lls_rna(arma::sp_mat& mat, arma::vec& s, arma::vec& x, arma::vec& bg, int& size_dnb) {
...
// [[Rcpp::export]]
Rcpp::NumericVector
lls_protein(arma::sp_mat& mat, arma::vec& s, arma::vec& x, arma::vec& xsd, arma::vec& bg) { 
  Rcpp::NumericVector res(s.n_rows);
  arma::vec::const_iterator x_iter = x.begin();
  ...
  for(; x_iter != x.end(); ++x_iter) {
    ...
    for(; s_iter != s.end(); ++s_iter) {
      // double yhat = (*s_iter) * (*x_iter) + (*bg_iter);
      double yhat = (*s_iter) * (*x_iter);
      double sd = (*s_iter) * (*xsd_iter);
      int i = s_iter - s.begin();
      int j = x_iter - x.begin();
      res(i) += R::dnorm(mat(i, j), yhat, sd, 1);
      // ++bg_iter;
    }
    ++xsd_iter;
  }
